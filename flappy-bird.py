...
pipes = []
velocity_x = -2 # move pipes to the left speed (simulates bird moving forward)
velocity_y = 0 # bird's vertical velocity
gravity = 0.4
score = 0
game_over = False
...
def move():
    ...
    velocity_y += gravity
    bird.y += velocity_y
    bird.y = max(bird.y, 0) # prevents bird from going above the window
    ...
    for pipe in pipes:
        ...
        if not pipe.passed and bird.x > pipe.x + pipe.width:
            score += 0.5 # there are two pipes (0,5 * 2 = 1 point per pipe pair)
            pipe.passed = True
...
# create new pipes and add them to the pipes list
def create_pipes():
    ...
    print(len(pipes)) # for testing pipe positions purposes
    

# initialize pygame
pygame.init()
window = pygame.display.set_mode((GAME_WIDTH, GAME_HEIGHT))
pygame.display.set_caption("Flappy Bird by @lucsantosdev")
...
# maintaining the game through a main loop
while True:
